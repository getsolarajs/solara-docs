...
*   `$addSelectMenu[type;customID;placeholder;minValues?;maxValues?;disabled?]`: Starts building a string select menu. Add options using `$addSelectMenuOption`. Optional: min max values disable. Types: `user`, `string`, `role`, `mentionable` and `channel`.
